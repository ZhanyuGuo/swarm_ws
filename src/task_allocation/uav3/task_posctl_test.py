...
class MavrosOffboardPosctlTest(MavrosTestCommon):
    ...
    def setUp(self):
        ...
        self.raw_setpoint_pub = rospy.Publisher('/uav3/mavros/setpoint_raw/local', PositionTarget, queue_size=1)
        self.idle_pub = rospy.Publisher('/uav3_task_bool', Bool, queue_size=1)

        # subscribers
        self.mission_done_sub = rospy.Subscriber('/mission_bool', Bool, self.mission_done_callback)
        self.task_pos_sub = rospy.Subscriber('/uav3_curr_task', PoseStamped, self.task_pos_callback)
        # self.uav3_pos_sub = rospy.Subscriber('/uav3/mavros/local_position/pose', PoseStamped, self.pos_callback)
        self.raw_setpoint_sub = rospy.Subscriber('/uav3/mavros/setpoint_raw/bridge', PositionTarget, self.raw_callback)
...
if __name__ == '__main__':
    try:
        import rostest
        rospy.init_node('test_uav3_node', anonymous=True)
